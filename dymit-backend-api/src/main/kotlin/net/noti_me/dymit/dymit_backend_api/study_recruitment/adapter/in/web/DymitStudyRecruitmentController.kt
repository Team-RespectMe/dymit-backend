...
class DymitStudyRecruitmentController(
    ...
    override fun getStudyRecruitmentList(
        ...
        @RequestParam(defaultValue = "DYMIT") type: StudyRecruitmentRequestType,
        @RequestParam(defaultValue = "false") mine: Boolean,
        @LoginMember(required = false) memberInfo: MemberInfo?
    ): ListResponse<DymitStudyRecruitmentSummaryResponse> {
        val summaries = when ( type ) {
            StudyRecruitmentRequestType.DYMIT -> queryDymitRecruitments(
                memberInfo = memberInfo,
                cursor = cursor,
                size = size,
                mine = mine
            )
            StudyRecruitmentRequestType.EXTERNAL -> queryExternalUseCase.execute(
                QueryStudyRecruitmentCommand(cursor = cursor, size = size)
            ).map(DymitStudyRecruitmentSummaryDto::from)
        }
        val responses = summaries
    ...
    private fun queryDymitRecruitments(
        ...
        if ( mine && memberInfo == null ) {
            throw UnauthorizedException(message = "인증 정보가 필요합니다.")
        }
        return getListUseCase.execute(
            GetDymitStudyRecruitmentListQuery(
                ...
                size = size,
                mine = mine,
                memberId = memberInfo?.memberId.orEmpty()
            )
        )
    }
