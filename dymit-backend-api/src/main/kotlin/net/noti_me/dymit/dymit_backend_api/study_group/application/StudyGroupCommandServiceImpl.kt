...
import net.noti_me.dymit.dymit_backend_api.study_group.application.dto.command.ChangeGroupOwnerCommand
...
import net.noti_me.dymit.dymit_backend_api.study_group.application.dto.command.StudyGroupImageUpdateCommand
...
import net.noti_me.dymit.dymit_backend_api.study_group.application.dto.command.StudyGroupModifyCommand
...
class StudyGroupCommandServiceImpl(
    ...
    private val boardPort: StudyGroupBoardPort,
    ...
    : StudyGroupDto {
        ...
        val groupCount = loadStudyGroupPort.countByOwnerId(memberEntity.id)
        ...
        var studyGroup = StudyGroup(
            ...
            ownerId = ObjectId(memberEntity.id),
        )
        ...
        val owner = StudyGroupMember(
            ...
            memberId = ObjectId(memberEntity.id),
            nickname = memberEntity.nickname,
            profileImage = ProfileImageVo.of(
                type = memberEntity.profileImageType,
                url = memberEntity.profileImageThumbnail
            ),
            role = GroupMemberRole.OWNER
        )
        studyGroupMemberRepository.persist(owner)
        createStudyGroupBoard(studyGroup, "공지 사항")
    ...
    : StudyGroupMemberDto {
        ...
        if ( group.isBlackListed(member.id) ) {
...
//
    ...
    override fun leaveStudyGroup(
        ...
        applicationEventPublisher.publishEvent(GroupMemberLeaveEvent.of(
            group = group,
            member = membership
        ))
