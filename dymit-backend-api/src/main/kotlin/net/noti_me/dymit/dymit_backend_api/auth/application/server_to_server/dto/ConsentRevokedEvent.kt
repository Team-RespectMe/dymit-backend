package net.noti_me.dymit.dymit_backend_api.auth.application.server_to_server.dto

class ConsentRevokedEvent(
    type: String,
    sub: String,
    eventTime: Long
): AppleS2SEvent(type, sub, eventTime) {

    init {
        require(type == EVENT_TYPE ) { "Invalid event type: $type" }
    }

    companion object {
        const val EVENT_TYPE = "consent-revoked"

        fun from(payload: AppleS2SPayload): ConsentRevokedEvent {
            return ConsentRevokedEvent(
                type = payload.events["type"] as String,
                sub = payload.events["sub"] as String,
                eventTime = payload.events["event_time"] as Long
            )
        }
    }
}