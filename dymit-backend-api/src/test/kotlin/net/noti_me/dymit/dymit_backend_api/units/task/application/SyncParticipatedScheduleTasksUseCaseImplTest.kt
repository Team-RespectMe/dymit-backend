...
import net.noti_me.dymit.dymit_backend_api.task.application.SyncParticipatedScheduleTasksUseCaseImpl
        ...
        Given("일정 참여자 사전 과제 동기화") {
            When("PRE 과제가 여러 개 있고 일부 과제에만 신규 assignee가 추가되면") {
                Then("실제로 추가된 과제만 반환한다") {
                    ...
                    result.map { it.taskId } shouldContainExactly listOf(addedTask1.identifier, addedTask2.identifier)
                }
            }
