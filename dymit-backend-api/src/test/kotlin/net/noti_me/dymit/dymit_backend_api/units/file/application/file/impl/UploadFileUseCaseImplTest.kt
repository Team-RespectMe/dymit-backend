...
import net.noti_me.dymit.dymit_backend_api.file.application.S3FileService
import net.noti_me.dymit.dymit_backend_api.file.application.UploadFileUseCaseImpl
        ...
        Given("JPEG 파일 업로드 요청이 주어지면") {
            When("원본 파일과 썸네일 업로드가 모두 성공하면") {
                Then("썸네일 메타데이터를 저장하고 응답에 포함한다") {
                    ...
                    val result = useCase.execute(loginMember, command)
            ...
            When("썸네일 업로드가 실패하면") {
                Then("파일 상태를 FAILED 로 갱신하고 예외를 던진다") {
                    ...
                    shouldThrow<InternalServerError> {
                        useCase.execute(loginMember, command)
                    }
                    savedStatuses.shouldContainExactly(
                        UserFileStatus.REQUESTED,
