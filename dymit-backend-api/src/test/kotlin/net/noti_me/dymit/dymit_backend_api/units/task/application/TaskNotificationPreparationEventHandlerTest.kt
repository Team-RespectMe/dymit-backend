...
import net.noti_me.dymit.dymit_backend_api.task.application.TaskNotificationPreparationEventHandler
        ...
        Given("과제 생성 이벤트 핸들러") {
            When("생성 이벤트를 받으면") {
                Then("현재 assignee 기준으로 TaskCreatedBroadcastEvent를 발행한다") {
                    ...
                    published.toPersonalPushMessages().map { it.memberId } shouldContainExactly listOf(memberId1, memberId2)
        ...
        Given("과제 수정 이벤트 핸들러") {
            When("수정 이벤트를 받으면") {
                Then("현재 assignee 기준으로 TaskModifiedBroadcastEvent를 발행한다") {
                    ...
                    verify(exactly = 1) { eventPublisher.publishEvent(capture(slot)) }
                    val published = slot.captured
                    published.toPersonalPushMessages().map { it.memberId } shouldContainExactly listOf(memberId)
                    published.toPersonalPushMessages().single().eventName shouldBe "TASK_MODIFIED"
                }
            }
        }
