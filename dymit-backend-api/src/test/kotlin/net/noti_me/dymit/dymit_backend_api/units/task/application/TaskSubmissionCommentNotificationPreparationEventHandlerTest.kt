        ...
        Given("과제 제출 댓글 생성 이벤트 핸들러") {
            When("댓글 생성 이벤트를 받으면") {
                Then("제출자 대상 TaskSubmissionCommentCreatedBroadcastEvent를 발행한다") {
                    ...
                    published.toPersonalPushMessages().map { it.memberId } shouldContainExactly listOf(assigneeMemberId)
                    published.toPersonalFeedData().single().resources.map { it.resourceId } shouldContainExactly listOf(
                        group.identifier,
                        task.identifier,
                        assigneeMemberId.toHexString()
                    )
                    published.toPersonalPushMessages().single().data shouldBe mapOf(
