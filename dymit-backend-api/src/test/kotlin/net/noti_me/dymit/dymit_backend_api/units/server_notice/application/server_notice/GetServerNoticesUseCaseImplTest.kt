...
import net.noti_me.dymit.dymit_backend_api.server_notice.application.GetNoticesUseCaseImpl
    ...
    init {
        ...
        Given("공지들을 조회할 때") {
            When("cursor 없이 size만 주어지면") {
                ...
                Then("처음부터 조회된다") {
                    val results = usecase.execute(cursor, size)
            ...
            When("커서를 주고 조회를 하면") {
                ...
                Then("커서 직후 부터 조회되어야 한다.") {
                    val results = usecase.execute(cursor.toHexString(), size)
                    results.size shouldBe size
                    results.forEachIndexed { index, notice ->
