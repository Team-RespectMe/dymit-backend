...
import net.noti_me.dymit.dymit_backend_api.application.board.v2.impl.GetBoardPostsUseCaseImplV2
...
internal class GetBoardPostsUseCaseImplV2Test : BehaviorSpec({
    ...
    val useCase = GetBoardPostsUseCaseImplV2(
    ...
    fun createPost(title: String, createdAt: LocalDateTime, writerMember: StudyGroupMember): Post {
        return Post(
            id = ObjectId.get(),
            groupId = groupId,
            boardId = boardId,
    ...
    given("게시글 V2 목록 조회 시") {
        `when`("정상 요청이면") {
            then("게시글을 생성일시 내림차순으로 반환한다") {
                val member = createGroupMember()
                val oldPost = createPost("old", LocalDateTime.now().minusDays(1), member)
                val newPost = createPost("new", LocalDateTime.now(), member)
                ...
                every {
                    postRepository.findByBoardIdLteId(
                        boardId = boardId.toHexString(),
                        lastId = null,
                        limit = 20,
                        category = PostCategory.QUESTION
                    )
                } returns listOf(oldPost, newPost)
                ...
                result.first().title shouldBe "new"
                verify(exactly = 1) {
                    postRepository.findByBoardIdLteId(boardId.toHexString(), null, 20, PostCategory.QUESTION)
                }
            }
        }
